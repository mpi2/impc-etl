...
def dcc_experiment_df(spark_session):
    if os.path.exists(FIXTURES_PATH + 'dcc_experiment_df'):
        dcc_experiment_df = spark_session.read.parquet(FIXTURES_PATH + 'dcc_experiment_df')
    else:
        dcc_experiment_df = extract_dcc_xml_files(spark_session, INPUT_PATH, 'experiment')
        dcc_experiment_df.write.mode('overwrite').parquet(FIXTURES_PATH + 'dcc_experiment_df')
...
def dcc_specimen_df(spark_session):
    if os.path.exists(FIXTURES_PATH + 'dcc_specimen_df'):
        ...
        dcc_specimen_df.write.mode('overwrite').parquet(FIXTURES_PATH + 'dcc_specimen_df')
...
class TestDCCGetByType:
    ...
    def test_number_of_experiments(self, dcc_experiment_df, entity_type):
        ...
        assert get_experiments_by_type(dcc_experiment_df, entity_type).count() == expected
