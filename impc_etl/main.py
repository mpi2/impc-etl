...
def main():
    ...
    parser.add_argument(
        "-d, --dcc", dest="dcc_path", nargs=1, help="DCC XML files path"
    )
    parser.add_argument(
        "-e, --env", dest="exec_env", nargs=1, help="Execution environment"
    )
    ...
    impress_df = impress_e.extract_impress(
        spark, impress_api_url, impress_pipeline_type
    )
    ...
    imits_df = imits_e.extract_phenotyping_colonies(
        spark, base_path + "data/imits/imits-report.tsv"
    )
    ...
    if file_exists(spark.sparkContext, base_path + "out/dcc_xml_spe_df.parquet"):
        ...
        dcc_xml_spe_df.write.mode("overwrite").parquet(
            base_path + "out/dcc_xml_spe_df.parquet"
        )
    ...
    if file_exists(spark.sparkContext, base_path + "out/dcc_xml_exp_df.parquet"):
        ...
        dcc_xml_exp_df.write.mode("overwrite").parquet(
            base_path + "out/dcc_xml_exp_df.parquet"
        )
    ...
    experiment_df = dcc_t.process_experiments(
        experiment_df, mice_df, embryo_df, impress_df
    )

    logger.info("Experiments cleaning finished")

    if file_exists(
        spark.sparkContext, base_path + "out/dcc_xml_exp_df_derivation.parquet"
    ):
        logger.info("Found parquet file for derived parameters data")
        experiment_df = spark.read.parquet(
            base_path + "out/dcc_xml_exp_df_derivation.parquet"
        )
    else:
        logger.info(
            "Not found parquet file for experiments data, calculating derived parameters"
        )
        experiment_df = dcc_t.get_derived_parameters(spark, experiment_df, impress_df)
        experiment_df = experiment_df.withColumnRenamed(
            "Phenotyping Consortium", "_phenotypingConsortium"
        )
